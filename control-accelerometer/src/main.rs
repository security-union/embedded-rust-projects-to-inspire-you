...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for cs_handle in &mut cs_handles {
        ...
        println!("ADXL345 on CS pin {:?} has device ID: {:?}", cs_handle, device_id);
        if device_id[0] != 0xE5 {
...
fn write_register(spi: &mut Spidev, cs: &mut LineHandle, reg_address: u8, data: u8) -> Result<(), Box<dyn std::error::Error>> {
...
fn read_register(spi: &mut Spidev, cs: &mut LineHandle, reg_address: u8, length: usize) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    cs.set_value(0)?; // Set CS low
    let command = vec![reg_address | REG_READ | (if length > 1 { REG_MULTI_BYTE } else { 0 })];
    let tx_buf = command.iter().cloned().chain(std::iter::repeat(0).take(length)).collect::<Vec<_>>();
...
fn read_acceleration(spi: &mut Spidev, cs: &mut LineHandle) -> 
